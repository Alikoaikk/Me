(function () {
  ...
  function initHeroFluid() {
    ...
    const palette = ['108,99,255', '0,212,255', '160,50,255', '0,180,255', '130,80,255'];

    // Slow base orbs that lazily follow the mouse
    const baseOrbs = [
      { x: 0, y: 0, speed: 0.04, r: 500, color: '108,99,255', opacity: 0.22, ox: -160, oy: -90 },
      { x: 0, y: 0, speed: 0.025, r: 420, color: '0,212,255', opacity: 0.18, ox:  160, oy: 100 },
      { x: 0, y: 0, speed: 0.06, r: 340, color: '160,50,255', opacity: 0.15, ox:  20,  oy: -120 },
    ];

    // Trail particles spawned on mouse move
    const trail = [];
    ...
    function resize() {
      ...
      baseOrbs[0].x = canvas.width * 0.2;  baseOrbs[0].y = canvas.height * 0.5;
      baseOrbs[1].x = canvas.width * 0.8;  baseOrbs[1].y = canvas.height * 0.5;
      baseOrbs[2].x = canvas.width * 0.5;  baseOrbs[2].y = canvas.height * 0.35;
    }
    ...
    window.addEventListener('mousemove', e => {
      ...
      trail.push({
        ...
        y: mouse.y,
        r: 120 + speed * 2,
        opacity: 0.12 + Math.min(speed * 0.003, 0.12),
    ...
    function draw() {
      ...
      for (let i = trail.length - 1; i >= 0; i--) {
        ...
        g.addColorStop(0, `rgba(${p.color},${p.opacity * p.life})`);
